export const featuredPlaces = {
  hotels: [
    {
      id: 'h1',
      name: 'Victoria Cần Thơ Resort',
      description: 'Khu nghỉ dưỡng 5 sao mang đậm kiến trúc Đông Dương, tọa lạc bên bờ sông Hậu thơ mộng.',
      address: 'Khu Bãi Cát, Phường Cái Khế, Quận Ninh Kiều, TP. Cần Thơ',
      image: '/images/hotel_victoria_1782101770881.png',
      rating: 5,
      tags: ['Sang trọng', 'Ven sông', 'Spa'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Victoria+Can+Tho+Resort,+1+Cach+Mang+Thang+Tam,+Can+Tho'
    },
    {
      id: 'h2',
      name: 'Mường Thanh Luxury Cần Thơ',
      description: 'Khách sạn 5 sao cao cấp đầu tiên tại Đồng bằng sông Cửu Long, tầm nhìn bao quát toàn thành phố.',
      address: 'Khu E1, Cồn Cái Khế, Phường Cái Khế, Quận Ninh Kiều, Thành phố Cần Thơ',
      image: '/images/hotel_muong_thanh_1782101782352.png',
      rating: 5,
      tags: ['Hồ bơi', 'Trung tâm', 'View đẹp'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Muong+Thanh+Luxury+Can+Tho+Hotel'
    },
    {
      id: 'h3',
      name: 'Azerai Cần Thơ',
      description: 'Resort 5 sao siêu sang biệt lập trên Cồn Ấu, không gian xanh mát và yên bình tuyệt đối.',
      address: 'Cồn Ấu, Phường Hưng Phú, Cái Răng, Cần Thơ',
      image: '/images/hotel_azerai_1782101796686.png',
      rating: 4.9,
      tags: ['Cồn Ấu', 'Sinh thái', 'Nghỉ dưỡng'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Azerai+Can+Tho+Resort'
    },
    {
      id: 'h4',
      name: 'Khách sạn Iris Cần Thơ',
      description: 'Khách sạn 4 sao nằm ngay trung tâm, thuận tiện tham quan chợ đêm và Bến Ninh Kiều.',
      address: '224 Đường 30 Tháng 4, Xuân Khánh, Ninh Kiều, Cần Thơ',
      image: '/images/hotel_iris_1782101810876.png',
      rating: 4.5,
      tags: ['Trung tâm', 'Gia đình', 'Tiện nghi'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Iris+Can+Tho+Hotel,+224+Duong+30+Thang+4,+Can+Tho'
    },
    {
      id: 'h5',
      name: 'Nam Bộ Boutique Hotel',
      description: 'Khách sạn boutique phong cách nhà cổ Nam Bộ, tọa lạc ngay trung tâm thành phố, gần Bến Ninh Kiều.',
      address: 'Số 01, Ngô Quyền, Phường Tân An, Quận Ninh Kiều, Thành phố Cần Thơ ',
      image: '/images/hotel_phuong_nam_1782210972310.png',
      rating: 4.4,
      tags: ['Boutique', 'Trung tâm', 'Nhà cổ'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Nam+Bo+Boutique+Hotel+Can+Tho'
    },
    {
      id: 'h6',
      name: 'Ninh Kiều Riverside Hotel',
      description: 'Khách sạn ấm cúng nội thất gỗ, view sông Hậu, cách Bến Ninh Kiều chỉ 2 phút đi bộ.',
      address: '2 Hai Bà Trưng, Tân An, Ninh Kiều, Cần Thơ',
      image: '/images/hotel_ninh_kieu_hotel_1782210925324.png',
      rating: 4.3,
      tags: ['Ấm cúng', 'View sông', 'Gia đình'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Ninh+Kieu+Riverside+Hotel,+2+Hai+Ba+Trung,+Can+Tho'
    }
  ],
  restaurants: [
    {
      id: 'r1',
      name: 'Lẩu mắm Dạ Lý',
      description: 'Quán lẩu mắm danh tiếng nhất nhì Cần Thơ với hương vị đậm đà và hàng chục loại rau ăn kèm.',
      address: '89 Đường 3/2, Ninh Kiều, Cần Thơ',
      image: '/images/food_lau_mam_1782101821872.png',
      rating: 4.7,
      tags: ['Đặc sản', 'Lẩu mắm', 'Lâu đời'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Lau+Mam+Da+Ly,+89+Duong+3+Thang+2,+Can+Tho'
    },
    {
      id: 'r2',
      name: 'Vịt nấu chao Thành Giao',
      description: 'Món vịt nấu chao trứ danh hẻm 1, từng đón nhiều nghệ sĩ nổi tiếng đến thưởng thức.',
      address: 'Hẻm 1 Lý Tự Trọng, Ninh Kiều, Cần Thơ',
      image: '/images/food_vit_nau_chao_1782101834624.png',
      rating: 4.5,
      tags: ['Ăn tối', 'Vịt nấu chao', 'Bình dân'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Vit+nau+chao+Thanh+Giao,+Ly+Tu+Trong,+Can+Tho'
    },
    {
      id: 'r3',
      name: 'Bánh xèo 7 Tới',
      description: 'Quán bánh xèo nổi tiếng lâu năm tại Cần Thơ, vỏ bánh giòn rụm, nhân tôm thịt tươi ngon, ăn kèm nước mắm chua ngọt đặc trưng.',
      address: '45 Hoàng Quốc Việt, phường An Bình, quận Ninh Kiều, Cần Thơ ',
      image: '/images/food_banh_xeo_muoi_xiem_1782211216815.png',
      rating: 4.5,
      tags: ['Bánh xèo', 'Lâu năm', 'Đặc sản'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Banh+xeo+7+Toi,+De+Tham,+Ninh+Kieu,+Can+Tho'
    },
    {
      id: 'r4',
      name: 'Nhà hàng Hoa Sứ',
      description: 'Nhà hàng ven sông chuyên phục vụ các món ăn Nam Bộ dân dã như ốc bươu nướng tiêu, cá lóc nướng trui.',
      address: 'Bến Ninh Kiều, Ninh Kiều, Cần Thơ',
      image: 'https://res.cloudinary.com/dnnz4ze3b/image/upload/v1773458538/kham-pha-2khudulich-con-noi-tieng-ocantho-06-7-2018-6_ywyh6f.jpg',
      rating: 4.6,
      tags: ['Nhà hàng', 'Ven sông', 'Hải sản'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Nha+hang+Hoa+Su,+Ben+Ninh+Kieu,+Can+Tho'
    },
    {
      id: 'r5',
      name: 'Nem Nướng Thanh Vân - Cần ',
      description: 'Nem nướng thơm lừng, ăn kèm bánh hỏi, chả giò và đa dạng các loại rau sống.',
      address: '17 Đại lộ Hòa Bình, Q. Ninh Kiều, Cần Thơ',
      image: '/images/food_bun_ca_chau_doc_1782210939989.png',
      rating: 4.5,
      tags: ['Nem nướng', 'Cả ngày', 'Đặc sản'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Bun+ca+Chau+Doc+Can+Tho'
    },
    {
      id: 'r6',
      name: 'Bánh mì bơ cốm',
      description: 'Quán bánh mì nổi tiếng với nước sốt bơ cốm béo thơm.',
      address: '66A Mạc Thiên Tích, Phường Xuân Khánh, Quận Ninh Kiều Cần Thơ',
      image: '/images/food_banh_mi_bay_ho_1782210952088.png',
      rating: 4.3,
      tags: ['Bánh mì', 'Sáng', 'Vỉa hè'],
      mapUrl: 'https://www.google.com/maps/search/?api=1&query=Banh+mi+Bay+Ho,+Nguyen+Trai,+Ninh+Kieu,+Can+Tho'
    }
  ]
};
